#ifndef INCLUDE_COUNTING_BLOOM_FILTER_H
#define INCLUDE_COUNTING_BLOOM_FILTER_H
...
public:
  ...
  /*counting_bloom_filter(long unsigned int tbl_size, long unsigned int elem_size, unsigned int fun_size) {
    counting_bloom_filter_init(tbl_size, elem_size, fun_size);
    }*/

  counting_bloom_filter() {hash_table = NULL;}
  
  void counting_bloom_filter_init(long unsigned int tbl_size, long unsigned int elem_size, unsigned int fun_size) {
    table_size = tbl_size;
    if (table_size == 0) table_size = 1;
    //cout << " counting_bloom_filter_init: table size " << tbl_size << endl;
   ...
   void remove(const EtherAddress & p)
   {
     ...
     srand((p.addr[1]<<16)+p.addr[2]);
      for(unsigned int i = 0; i < function_size; i++) {
        long unsigned int hash =  (long unsigned int) (rand()/(double)RAND_MAX * table_size);
        //cout << hash << endl;
        if (hash_table[hash]) hash_table[hash] --;
        //else hash_table[hash] ++;
      }
   }
